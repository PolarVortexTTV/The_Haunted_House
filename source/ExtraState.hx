...
{
	public static var songs:Array<NewSongMetadata> = [];
	...
	{
		clean();
		var initSonglist = CoolUtil.coolTextFile(Paths.txt('data/extraSonglist'));
		...
		for (i in 0...initSonglist.length)
		{
			...
			var meta = new NewSongMetadata(data[0], Std.parseInt(data[2]), data[1]);
			...
			if (diffsThatExist.contains("Easy"))
				ExtraState.loadDiff(0,format,meta.songName,diffs);
			if (diffsThatExist.contains("Normal"))
				ExtraState.loadDiff(1,format,meta.songName,diffs);
			if (diffsThatExist.contains("Hard"))
				ExtraState.loadDiff(2,format,meta.songName,diffs);
			...
			ExtraState.songData.set(meta.songName,diffs);
			...
			if (FileSystem.isDirectory("assets/sm/" + i))
			{
				...
				for (file in FileSystem.readDirectory("assets/sm/" + i))
				{
					...
					if (file.endsWith(".sm") && !FileSystem.exists("assets/sm/" + i + "/converted.json"))
					{
						...
						var meta = new NewSongMetadata(file.header.TITLE, 0, "sm",file,"assets/sm/" + i);
						songs.push(meta);
		...
		var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('extrasbw'));
	...
	{
		songs.push(new NewSongMetadata(songName, weekNum, songCharacter));
	}
