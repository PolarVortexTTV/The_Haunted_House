	...
	**/
	...
	override function create()
	{
		...
		for (awfgaw in 0...Math.round(targetY / 640) + 1920) // grids/steps
		{
			...
			if(_song.mania == 1)
			{
				renderer = new SectionRender(0,640 * awfgaw, GRID_SIZE + 7);
			}
			else
			{
				renderer = new SectionRender(0,640 * awfgaw, GRID_SIZE + 0);
			}
		...
		strumLine = new FlxSprite(0, 0).makeGraphic(Std.int(GRID_SIZE * 2), Std.int(amountofnotes/2));
		...
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE + 10, GRID_SIZE * amountofnotes, GRID_SIZE * 80);
	...
	function regenerateLines()
	{
		...
		if (_song.eventObjects != null)
			for(i in _song.eventObjects)
			{
				...
				var line = new FlxSprite(0, pos).makeGraphic(Std.int(GRID_SIZE * 2), Std.int(amountofnotes/2), FlxColor.BLUE);
		...
		for (i in sectionRenderes)
		{
			...
			var line = new FlxSprite(0, pos).makeGraphic(Std.int(GRID_SIZE * 2), Std.int(amountofnotes/2), FlxColor.BLACK);
			line.alpha = 0.4;
			lines.add(line);
		}
	}
	...
	function addGrid(?divisions:Float = 1)
	{
		...
		var sections = Math.floor(((lengthInSteps + 16)));
		remove(gridBG);
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE + 10, GRID_SIZE * amountofnotes, GRID_SIZE * sections);
		trace(gridBG.height);
	...
	{
		var UI_songTitle = new FlxUIInputText(10, 10, 70, _song.song, 10);
		typingShit = UI_songTitle;

		var check_4keys = new FlxUICheckBox(50, 50, null, null, "Toggles 5 key", 100, null);
		if(_song.mania == 1)
		{
			updatedGrid = false;
			check_4keys.checked = true;
			amountofnotes = 10;
			addGrid();
			updateGrid();
		}
		check_4keys.callback = function()
		{
			updatedGrid = false;
			if(check_4keys.checked)
			{
				_song.mania = 1;
				amountofnotes = 10;
				addGrid();
		...
		var saveButton:FlxButton = new FlxButton(110, 10, "Save", function()
		{
			saveLevel();
		});
		...
		tab_group_song.add(check_4keys);
	...
	function addNoteUI():Void
	{
		...
		stepperSusLength = new FlxUINumericStepper(10, 10, Conductor.stepCrochet / 2, 0, 0, Conductor.stepCrochet * 20 * 4);
		...
		for(i in array)
			{
				...
				for(ii in _song.notes)
				{
					if (ii.startTime <= strum && ii.endTime > strum)
					{
						...
						var note:Note = new Note(strum, Math.floor(i[1] % 5),null,false,true,i[3], i[4]);
						...
						if (note.sustainLength > 0)
							{
								var sustainVis:FlxSprite = new FlxSprite(note.x + (GRID_SIZE / 2),
									note.y + GRID_SIZE).makeGraphic(10, Math.floor((getYfromStrum(note.strumTime + note.sustainLength) * zoomFactor) - note.y));
	...
	function poggers()
	{
		...
		for(i in curRenderedNotes)
		{
			...
			if (i.noteCharterObject != null)
			{
				...
				i.noteCharterObject.makeGraphic(10,Math.floor((getYfromStrum(i.strumTime + i.sustainLength) * zoomFactor) - i.y),FlxColor.WHITE);
			}
	...
	{
		var sections = Math.floor(((lengthInSteps + 16)));
		if(_song.mania == 1 && !updatedGrid)
		{
			updatedGrid = true;
			UI_options.x = UI_box.x;
			//remove(gridBG);
			gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * amountofnotes, GRID_SIZE * sections);
			//add(gridBG);
		}
		else
		{
					...
					if (diff < 8000 && diff >= -8000)
					{
						...
						if (note.sustainLength > 0)
						{
							...
							if (note.noteCharterObject.y != note.y + GRID_SIZE)
							{
								...
								note.noteCharterObject.makeGraphic(10,Math.floor((getYfromStrum(note.strumTime + note.sustainLength) * zoomFactor) - note.y),FlxColor.WHITE);
							}
						}
						note.active = true;
						note.visible = true;
		...
		var rightO = FlxG.keys.justPressed.ZERO;
		...
		if (FlxG.keys.justPressed.V && !FlxG.keys.pressed.CONTROL)
			{
				...
				if (secit != null)
				{
					...
					for (i in 0...secit.sectionNotes.length)
					{
						...
						if (note[1] < Std.int(amountofnotes/2))
							note[1] += Std.int(amountofnotes/2);
						else
							note[1] -= Std.int(amountofnotes/2);
						newSwaps.push(note);
					}
					...
					{
						for(ii in newSwaps)
							if (i.strumTime == ii[0] && i.noteData == ii[1] % Std.int(amountofnotes/2))
							{
	...
	function changeNoteSustain(value:Float):Void
	{
		if (curSelectedNote != null)
		{
			if (curSelectedNote[2] != null)
			{
				...
				curSelectedNoteObject.y + GRID_SIZE).makeGraphic(10, Math.floor((getYfromStrum(curSelectedNoteObject.strumTime + curSelectedNote[2]) * zoomFactor) - curSelectedNoteObject.y));
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % Std.int(amountofnotes/2), daType);
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n == null)
		{
			var note:Note = new Note(noteStrum, noteData % Std.int(amountofnotes/2),null,false,true,TimingStruct.getBeatFromTime(noteStrum), noteType);
		...
		{
			var note:Note = new Note(n.strumTime, n.noteData % Std.int(amountofnotes/2),null,false,true, n.isAlt,TimingStruct.getBeatFromTime(n.strumTime), n.noteType);
