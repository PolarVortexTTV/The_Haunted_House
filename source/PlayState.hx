...
class PlayState extends MusicBeatState
{
	...
	public static var isExtraMode:Bool = false;
	...
	public static var dadXOffset:Int = 0;
	public static var dadYOffset:Int = 0;

	public static var bfXOffset:Int = 0;
	public static var bfYOffset:Int = 0;

	public static var diabloFALLING:FlxSprite;
	...
	public static var eventBool:Bool = false;
	public static var eventExecution:Bool = false;
	...
	private var dataSuffix2:Array<String> = ['LEFT', 'DOWN', 'SPACE', 'UP', 'RIGHT'];
	private var dataColor2:Array<String> = ['purple', 'blue', 'yellow', 'green', 'red'];
	public var number = 4; // This is for the amount of keys.
	...
	override public function create()
	{
		...
		{
			number = 5;
			noteBools = [false, false, false, false, false];
		}
			...
			if (!PlayStateChangeables.Optimize)
				switch (Stage.curStage)
				{
					...
					case 'mansion-evil':
						...
						boyfriend.x = 2124;
						boyfriend.y = 991;
						boyfriend.scrollFactor.set(.9, .9);
						gf.scrollFactor.set(.9, .9);
						gf.angle = 4;
					case 'fireplace':
						gf.scrollFactor.set(0.9, 0.9);
						if(dad.curCharacter == "battered")
						{
         ...
         switch (SONG.player2)
           {
			 ...
			 case 'ghoul':
			  healthBar.createFilledBar(0xFFF752F7, 0xFF0097C4);
			 case 'angryghoul':
			  healthBar.createFilledBar(0xFFF752F7, 0xFF0097C4);	
			 case 'soulfire':
			  healthBar.createFilledBar(0xFFF752F7, 0xFF0097C4);	
			 case 'diablo':
			  healthBar.createFilledBar(0xFFFF0000, 0xFF0097C4);
			 case 'epsilon':
			  healthBar.createFilledBar(0xFFFFFFFF, 0xFFF752F7);
			 case 'battered':
			  healthBar.createFilledBar(0xFF000000, 0xFF0097C4);
			 case 'voodoo':
			  healthBar.createFilledBar(0xFFFFFFFF, 0xFF0097C4);
			 case 'matt':
			  healthBar.createFilledBar(0xFFDE8222, 0xFF0097C4);
			 case 'sonic':
			  healthBar.createFilledBar(0xFF0031AD, 0xFFF752F7);
			 case 'ezra':
			  healthBar.createFilledBar(0xFF0031AD, 0xFFFFFFFF);
		     case 'jester':
			  healthBar.createFilledBar(0xFF240927, 0xFF0097C4);
			 case 'obama':
			  healthBar.createFilledBar(0xFFFFFFFF, 0xFF0097C4);
            }
        }
	...
	function voodooIntro():Void
	{
		var newIntro:FlxSprite = new FlxSprite(0, 0);
		newIntro.frames = Paths.getSparrowAtlas('voodoo/Spotlight', 'shared');
		newIntro.animation.addByPrefix('idle', 'Spotlight instance 1', 24, false);
		camFollow.x += 200;
		newIntro.screenCenter();
		newIntro.x += 600;
		newIntro.scrollFactor.set();
		newIntro.animation.play('idle');
		trace('VOODOO INTRO!!!');
		add(newIntro);
		new FlxTimer().start(4, function(tmr:FlxTimer)
		{
			startCountdown();
			newIntro.visible = false;
			newIntro.active = false;
		});
	}
	...
	private function releaseInput(evt:KeyboardEvent):Void // handles releases
	{
		...
		if(number == 5)
			{
				var oldKeys = keys;
				keys = [oldKeys[0], oldKeys[1], oldKeys[2], oldKeys[3], oldKeys[4]];
				binds = [
					FlxG.save.data.leftBind,
					FlxG.save.data.downBind,
	...
	public function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (daStrumTime < 0)
					daStrumTime = 0;
				var daNoteData:Int = Std.int(songNotes[1] % number);

				var gottaHitNote:Bool = true;

				if (songNotes[1] > number - 1 && section.mustHitSection)
					gottaHitNote = false;
				else if (songNotes[1] < number && !section.mustHitSection)
					gottaHitNote = false;
		...
		for (i in 0...number)
		{
			...
			switch (noteTypeCheck)
			{
				case 'epsilon':
					if(number == 5)
						{
							babyArrow.frames = Paths.getSparrowAtlas('Five_NOTES_ASSETS_BW');
							for (j in 0...number)
								{
									babyArrow.animation.addByPrefix(dataColor2[j], 'arrow' + dataSuffix2[j]);	
									babyArrow.animation.addByPrefix('dirCon' + j, dataSuffix2[j].toLowerCase() + ' confirm', 24, false);
								}
		
							var lowerDir:String = dataSuffix2[i].toLowerCase();

							babyArrow.animation.addByPrefix('static', 'arrow' + dataSuffix2[i]);
				...
				default:
					if(number == 5)
					{
						babyArrow.frames = Paths.getSparrowAtlas('Five_NOTES_ASSETS');
						for (j in 0...number)
							{
								babyArrow.animation.addByPrefix(dataColor2[j], 'arrow' + dataSuffix2[j]);	
								babyArrow.animation.addByPrefix('dirCon' + j, dataSuffix2[j].toLowerCase() + ' confirm', 24, false);
	...
	override public function update(elapsed:Float)
	{
		...
		switch(curSong)
		{
			case 'Agony':
			{
				if((Conductor.songPosition > 97400 && Conductor.songPosition < 98700))
				{
					eventBool = true;
					if(dad.curCharacter == 'soulfire' && !eventExecution)
					{
						eventExecution = true;
	
						dad.playAnim('roar', true);
	
						previousCamZoom = Stage.camZoom;
						new FlxTimer().start(0.5, function(tmr:FlxTimer)
						{
							FlxG.camera.zoom = FlxMath.lerp(Stage.camZoom, 1.25, 1);
						});
					}
				...
				else
				{
					/*if(eventBool)
					{
						FlxG.camera.zoom = FlxMath.lerp(Stage.camZoom, previousCamZoom, 1);
					}*/
					eventBool = false;
					eventExecution = false;
					//eventAnimationSprite.visible = false;
					dad.visible = true;
				}
			}
			case 'Soulpurgation':
			{
				if((Conductor.songPosition > 138460 && Conductor.songPosition < 139760))
					{
						eventBool = true;
						if(dad.curCharacter == 'soulfire' && !eventExecution)
						{
			...
			case 'Decay':
			{
				if((Conductor.songPosition > 135790 && Conductor.songPosition < 140700))
				{
					...
					if(dad.curCharacter == 'diablo' && !eventExecution)
					{
						...
						dad.visible = false;
						diabloFALLING = new FlxSprite(dad.x, dad.y);
						diabloFALLING.frames = Paths.getSparrowAtlas('characters/Diablo_Falling_Apart', 'shared');
						diabloFALLING.animation.addByPrefix('idle', 'Diablo Fall Apart', 24, false);
						diabloFALLING.animation.play('idle');
						add(diabloFALLING);
		...
		{
			if(dad.animation.curAnim.name.contains("roar") && dad.animation.finished)
			{
				FlxG.camera.zoom = FlxMath.lerp(Stage.camZoom, previousCamZoom, 1);
			}
		};
		...
		if (generatedMusic && currentSection != null)
		{
			...
			if (camFollow.x != dad.getMidpoint().x + 150 + dadXOffset && !currentSection.mustHitSection)
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
				...
				#end
				camFollow.setPosition(dad.getMidpoint().x + 150 + dadXOffset, dad.getMidpoint().y - 100 + dadYOffset);
				#if cpp
				if (luaModchart != null)
			...
			if (currentSection.mustHitSection && camFollow.x != boyfriend.getMidpoint().x - 100 + bfXOffset)
			{
				var offsetX = 0;
				var offsetY = 0;
				#if cpp
				...
				#end
				camFollow.setPosition(boyfriend.getMidpoint().x - 100 + bfXOffset, boyfriend.getMidpoint().y - 100 + bfYOffset);
		...
		if (generatedMusic)
		{
			...
			var holdArray2:Array<Bool> = [controls.LEFT, controls.DOWN, controls.MIDDLE, controls.UP, controls.RIGHT];
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.modifiedByLua)
				{
					switch(curSong)
					{
						case 'Sweep' | 'Escape' | 'Disintegrate' | 'Stitched':
						{
							if(Math.floor(Math.abs(daNote.noteData)) == 2 || Math.floor(Math.abs(daNote.noteData)) == 3)
							{
								if (daNote.mustPress)
									{
									...
									if (daNote.isSustainNote)
									{
										...
										if (!PlayStateChangeables.botPlay)
										{
											if(number == 5)
											{
												if ((!daNote.mustPress || daNote.wasGoodHit || holdArray2[Math.floor(Math.abs(daNote.noteData))])
				...
				if (!daNote.mustPress && Conductor.songPosition >= daNote.strumTime)
				{
					...
					if (!daNote.isParent && daNote.parent != null)
					{
						if (daNote.spotInLine != daNote.parent.children.length - 1)
						{
							...
							var customSuffix:Array<String> = ['LEFT', 'DOWN', 'UP', 'UP', 'RIGHT'];
							...
							if(!eventBool)
							{
								if(number == 5)
								{
									dad.playAnim('sing' + customSuffix[singData] + altAnim, true);
								}
								else
								{
						...
						if (daNote.isSustainNote && daNote.wasGoodHit)
							...
							else
							{
								if (loadRep && daNote.isSustainNote)
								{
									...
									{
										if(daNote.noteType != 1 && daNote.noteType != 3 && daNote.noteType != 4)
											vocals.volume = 0;
										if (theFunne && !daNote.isSustainNote)
										{
											noteMiss(daNote.noteData, daNote);
										}
										if (daNote.isParent && daNote.noteType != 1 && daNote.noteType != 3 && daNote.noteType != 4)
										{
										...
										else
										{
												...
												&& daNote.spotInLine != daNote.parent.children.length)
											{
												...
												&& !daNote.isSustainNote && daNote.noteType != 1 && daNote.noteType != 3 && daNote.noteType != 4)
											{
												health -= 0.15;
											}
										}
								...
								else
								{
									...
									if (daNote.isParent && daNote.visible && daNote.noteType != 1 && daNote.noteType != 3 && daNote.noteType != 4)
									{
										health -= 0.15; // give a health punishment for failing a LN
										trace("hold fell over at the start");
										for (i in daNote.children)
			...
			if (isStoryMode)
			{
				...
				else
				{
					...
					if(curSong == 'Dismiss')
					{
						...
						video.playMP4(Paths.video('AGONY'));
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		var holdArray2:Array<Bool> = [controls.LEFT, controls.DOWN, controls.MIDDLE, controls.UP, controls.RIGHT];
		var pressArray2:Array<Bool> = [controls.LEFT_P, controls.DOWN_P, controls.MIDDLE_P, controls.UP_P, controls.RIGHT_P];
		var releaseArray2:Array<Bool> = [controls.LEFT_R, controls.DOWN_R, controls.MIDDLE_R,controls.UP_R, controls.RIGHT_R];
		var keynameArray2:Array<String> = ['left', 'down', 'space', 'up', 'right'];
		...
		if (luaModchart != null)
		{
			if(number == 5)
			{
				for (i in 0...pressArray2.length) {
					if (pressArray2[i] == true) {
						luaModchart.executeState('keyPressed', [keynameArray2[i]]);
					}
				};
				
				for (i in 0...releaseArray2.length) {
					if (releaseArray2[i] == true) {
						luaModchart.executeState('keyReleased', [keynameArray2[i]]);
					}
				};
			}
			else
		...
		#end
		var key5array:Array<Bool> = [false, false, false, false, false];
		var key4array:Array<Bool> = [false, false, false, false];
		...
		if (PlayStateChangeables.botPlay)
		{
			...
			{
				holdArray = key5array;
				pressArray = key5array;
				releaseArray = key5array;
			}
			else
			{
				holdArray = key4array;
				pressArray = key4array;
				releaseArray = key4array;
			}
		}

		var anas:Array<Ana> = [null, null, null, null];
		var anas2:Array<Ana> = [null, null, null, null, null];

		if(number == 5)
		{
			for (i in 0...pressArray2.length)
				if (pressArray2[i])
					anas2[i] = new Ana(Conductor.songPosition, null, false, "miss", i);
	
			// HOLDS, check for sustain notes
			if (holdArray2.contains(true) && /*!boyfriend.stunned && */ generatedMusic)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					if (daNote.isSustainNote && daNote.canBeHit && daNote.mustPress && holdArray2[daNote.noteData] && daNote.sustainActive)
					{
						goodNoteHit(daNote);
					}
				});
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if(number == 5)
			{
				if (pressArray2.contains(true) && generatedMusic)
				{
					...
					if(number == 5)
					{
						...
						else if (possibleNotes.length > 0)
						{
							...
							{
								for (shit in 0...pressArray2.length)
								{ // if a direction is hit that shouldn't be
									if (pressArray2[shit] && !directionList.contains(shit))
										noteMiss(shit, null);
								}
							}
							for (coolNote in possibleNotes)
							{
								if (pressArray2[coolNote.noteData] && !hit[coolNote.noteData])
								{
									if (mashViolations != 0)
										mashViolations--;
									...
									var noteDiff:Float = -(coolNote.strumTime - Conductor.songPosition);
									anas2[coolNote.noteData].hit = true;
									anas2[coolNote.noteData].hitJudge = Ratings.judgeNote(noteDiff);
									anas2[coolNote.noteData].nearestNote = [coolNote.strumTime, coolNote.noteData, coolNote.sustainLength];
									goodNoteHit(coolNote);
								}
							}
						};
	
						if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray2.contains(true) || PlayStateChangeables.botPlay))
						{
	...
	{
		if (!boyfriend.stunned && daNote.noteType != 1 && daNote.noteType != 3 && daNote.noteType != 4)
		{
			...
			{
				var customSuffix:Array<String> = ['LEFT', 'DOWN', 'UP', 'UP', 'RIGHT'];
				boyfriend.playAnim('sing' + customSuffix[direction] + 'miss', true);
			}
			else
			{
			...
			switch(note.noteType)
			{
				case 4:
					if(!PlayStateChangeables.botPlay)
					{
						...
						if(number == 5)
						{
							boyfriend.playAnim('sing' + customSuffix[note.noteData] + 'miss', true);
						}
						else
						{
				...
				case 3:
					if(!PlayStateChangeables.botPlay)
					{
						...
						playerStrums.forEach(function(spr:FlxSprite)
						{
							...
							{
								var splash:FlxSprite = new FlxSprite(spr.x - spr.width + 45, spr.y - spr.height + 50);
								splash.frames = Paths.getSparrowAtlas("Trap_Splash");
								splash.animation.addByPrefix('splash', 'trap splash instance ', 24, false);
								splash.cameras = [camHUD];
								splash.animation.play('splash');
								FlxG.sound.play(Paths.sound('trap_splashsfx'));
								//splash.setGraphicSize(Std.int(splash.width * 0.6));
								add(splash);
					...
					if(!PlayStateChangeables.botPlay)
					{
						if(FlxG.save.data.missSounds)
							...
							playerStrums.forEach(function(spr:FlxSprite)
							{
								if(spr.ID == note.noteData)
								{
									var curse:FlxSprite = new FlxSprite(spr.x - spr.width - 10, spr.y - spr.height);
									switch(SONG.noteStyle)
									{
										default:
											curse.frames = Paths.getSparrowAtlas("GhostPoof");
										case 'epsilon':
											curse.frames = Paths.getSparrowAtlas("GhostPoof_BW");
									}
									curse.animation.addByPrefix('poof', 'ghoulplosion', 24, false);
									curse.cameras = [camHUD];
									curse.animation.play('poof');
									FlxG.sound.play(Paths.sound('ghostpoof'));
									curse.setGraphicSize(Std.int(curse.width * 0.6));
									add(curse);
									curse.animation.finishCallback = function(name:String)
									{
										remove(curse);
									}
				...
				case 2:
					playerStrums.forEach(function(spr:FlxSprite)
					{
						if(spr.ID == note.noteData)
						{
							var bone_break:FlxSprite = new FlxSprite(spr.x - spr.width + 25, spr.y - spr.height + 35);
							bone_break.frames = Paths.getSparrowAtlas("bone_break");
							bone_break.animation.addByPrefix('break', 'bone shatter instance', 24, false);
							bone_break.cameras = [camHUD];
							bone_break.animation.play('break');
							bone_break.setGraphicSize(Std.int(bone_break.width * 0.6));
							add(bone_break);
							bone_break.animation.finishCallback = function(name:String)
							{
								remove(bone_break);
							}
					...
					if(number == 5)
					{
						boyfriend.playAnim('sing' + customSuffix[note.noteData] + altAnim, true);
					}
					else
					{
	...
	override function beatHit()
	{
		...
		if(number == 5 && boyfriend.animation.curAnim.curFrame >= 10 && (boyfriend.animation.curAnim.name.startsWith("sing") || boyfriend.animation.curAnim.name.endsWith("miss")))
		{
			boyfriend.playAnim("idle");
		}
